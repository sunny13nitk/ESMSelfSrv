...
@Slf4j
public class CL_SrvUrlSIXSrv implements IF_SrvUrlSrv
{
    ...
    public String getSrvUrl(String caseid, String... destinationurl) throws EX_ESMAPI
    {
        ...
        if (StringUtils.hasText(caseid) && userSessSrv != null)
        {
            ...
            if (StringUtils.hasText(svyUrl) && StringUtils.hasText(caseid)
                    && CollectionUtils.isNotEmpty(userSessSrv.getCases4User4mSession()))
            {

                log.info(svyUrl + " : Before REplacement .....");

                Optional<TY_CaseESS> caseO = userSessSrv.getCases4User4mSession().stream()
                        .filter(c -> c.getId().equals(caseid)).findFirst();
                if (caseO.isPresent())
                {
                    svyUrl += (gc_number + caseO.get().getId());
                    svyUrl += (gc_priority + caseO.get().getPriority());
                    if (StringUtils.hasText(caseO.get().getCatg1()))
                    {
                        svyUrl += (gc_catg1 + caseO.get().getCatg1());
                    }
